...
to setup
  ...
  ; corta relvas: tamanho, forma, movimentos...
 ...
 ; pragas: tamanho, forma, movimentos...
  create-pragas 1[
    setxy random-pxcor random-pycor
     ;set heading random 360
...
;movimento das pragas:
  ask pragas [
    ; chama a função da probabilidade:
    if prob pragas_prob [
    ;rotações possíveis
    rt one-of [-90 90]
    move-to patch-ahead 1
    ;muda a cor para verde quando passar por patch castanhos:
    set pcolor green
...
;vai buscar a probabilidade ao deslizador:
to-report prob[x]
